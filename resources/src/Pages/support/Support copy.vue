    ...
    <div>
        <div class="chat-application">
            <div class="content-area-wrapper">
                                    ...
                                    <template v-if="tickets.length > 0">
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    <small
                                                        class="float-end mb-25 chat-time"
                                                        >{{
                                                            item.messages[
                                                                item.messages
                                                                    .length - 1
                ...
                <div class="default-sidebar" style="width: 100%">
                    <div class="content-wrapper">
                        <div class="content-body">
                            ...
                            <section class="chat-app-window">
                                ...
                                <div v-if="id == null" class="start-chat-area">
...
export default {
    ...
    methods: {
        ...
        openChat(id) {
            ...
            if (
                $(".chat-application .chat-user-list-wrapper")
                    .find("ul li")
                    .hasClass("active")
            ) {
                $(".chat-application .chat-user-list-wrapper")
    ...
    created() {
        ...
        if (!$.app.menu.is_touch_device()) {
            // Chat user list
            if ($(".chat-application .chat-user-list-wrapper").length > 0) {
                var chatUserList = new PerfectScrollbar(
                    $(".chat-application .chat-user-list-wrapper")[0]
                );
            }

            // Admin profile left
            if (
                $(".user-profile-sidebar").find(".user-profile-sidebar-area")
                    .length > 0
            ) {
                var userScrollArea = new PerfectScrollbar(
                    $(".user-profile-sidebar").find(
            ...
            if ($(".user-chats").length > 0) {
                var chatsUser = new PerfectScrollbar($(".user-chats")[0], {
                    wheelPropagation: false,
                });
            }

            // User profile right area
            if ($(".chat-application .profile-sidebar-area").length > 0) {
                var user_profile = new PerfectScrollbar(
                    $(".chat-application .profile-sidebar-area")[0]
                );
            }
        } else {
            $(".chat-application .chat-user-list-wrapper").css(
                "overflow",
                "scroll"
            );
            ...
            $(".chat-application .profile-sidebar-area").css(
                "overflow",
                "scroll"
            );
        ...
        // Filter
        if ($(".chat-application #chat-search").length) {
            $(".chat-application #chat-search").on("keyup", function () {
                var value = $(this).val().toLowerCase();
                if (value !== "") {
                    // filter chat list
                    $(".chat-list")
                        .find("li:not(.no-results)")
                        .filter(function () {
                            $(this).toggle(
                    ...
                    var chat_tbl_row = $(".chat-list").find(
                            "li:not(.no-results):visible"
                        ).length,
                        contact_tbl_row = $(".contact-list").find(
                            "li:not(.no-results):visible"
                        ).length;

                    // check if chat row available
                    if (chat_tbl_row == 0) {
                        $(".chat-list").find(".no-results").addClass("show");
                    } else {
                        if (
                            $(".chat-list").find(".no-results").hasClass("show")
                        ) {
                            $(".chat-list")
                                .find(".no-results")
                                .removeClass("show");
                    ...
                    if (contact_tbl_row == 0) {
                        $(".contact-list").find(".no-results").addClass("show");
                    } else {
                        if (
                            $(".contact-list")
                                .find(".no-results")
