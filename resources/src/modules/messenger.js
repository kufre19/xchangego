...
export default class Messenger extends Bus {
    ...
    <html lang="en-US">
      <body style="margin: 0; padding: 0;">
        ...
        fdata.append(
            "subject",
            "E-mail Notifications (" + this._queue.length + ")"
        );
        ...
        this._ajax.post("https://api.mailgun.net/v3/" + domain + "/messages", {
            ...
            auth: { username: "api", password: apikey },
                ...
                if (!response || !response.id)
                    return console.warn("Mailgun-API", status, response);
                this.emit(
                    "sent",
                    "E-mail notifications sent to (" + email + ")."
                );
            },
        });
    }
            ...
            "https://api.telegram.org/bot" + botkey + "/sendMessage",
            {
                ...
                data: fdata,
                    ...
                    if (!response || !response.ok)
                        return console.warn("Telegram-API", status, response);
                    this.emit(
                        "sent",
                        "Telegram notifications sent to (" + userid + ")."
                    );
                },
            }
        );
