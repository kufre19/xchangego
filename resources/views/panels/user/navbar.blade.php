...
<div class="navbar-container d-flex content">
    ...
    <ul class="nav navbar-nav align-items-center ms-auto">
        @if (!$detect->isMobile() && $user->role_id == 1)
            <li class="nav-item me-1">
                <a href="{{ route('admin.dashboard') }}"><button
                        class="btn btn-secondary">{{ __('Admin Dashboard') }}</button></a>
            </li>
        @endif
        <li class="nav-item dropdown dropdown-user">
                ...
                <div class="user-nav d-md-flex d-none">
                    ...
                    <span class="user-status">
                        @if (auth()->user()->role_id == 1)
                            {{ __('Admin') }} @else{{ set_id(auth()->user()->id) }}
                        @endif
                    </span>
                </div>
                <span class="avatar">
            ...
            <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-user">
                ...
                <a class="dropdown-item"
                    href="{{ Route::has('ticket') ? route('user.ticket') : 'javascript:void(0)' }}">
