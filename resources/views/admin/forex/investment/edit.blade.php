...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="card-title">{{ 'Editing '.$plan->title.' plan' }}</h4><div class="card-search"></div>
    </div>
    <form action="{{ route('admin.forex.investment.update') }}" method="POST" enctype="multipart/form-data" id="planUpdate">
        @csrf
            ...
            <div class="row">
                <div class="col-lg-3 col-md-4 col-sm-6">
                    ...
                    <div class="input-group mb-1">
                        <input type="text" class="form-control" id="title" name="title" aria-label="Plan Title" aria-describedby="title" value="{{ $plan->title }}">
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6">
                    <label class="form-label" for="price">{{ __('locale.Price')}}</label>
                    <div class="input-group mb-1">
                        <input type="text" class="form-control" id="price" name="price" aria-label="price" aria-describedby="password" value="{{ $plan->price }}">
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6">
                    <label class="form-label" for="duration">{{ __('locale.Duration (Days)')}}</label>
                    <div class="input-group mb-1">
                        <input type="text" class="form-control" id="duration" name="duration" aria-label="Duration" aria-describedby="duration" value="{{ $plan->duration }}">
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6">
                    <label class="form-label" for="roi">{{ __('locale.Return on Investment')}}</label>
                    <div class="input-group mb-1">
                        <input type="text" class="form-control" id="roi" name="roi" aria-label="Return on Investment" aria-describedby="roi" value="{{ $plan->roi }}">
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6">
                    ...
                    <div class="dropdown">
                        ...
                        @if ($plan->result_missed == 1){{ __('locale.Win')}}
                        @elseif ($plan->result_missed == 2){{ __('locale.Lose')}}
                        @elseif ($plan->result_missed == 3){{ __('locale.Draw')}}
                ...
                <div class="col-lg-3 col-md-4 col-sm-6">
                    ...
                    <div class="input-group mb-1">
                        <input type="number" class="form-control"  id="profit_missed" name="profit_missed" aria-label="Maximum Profit" aria-describedby="profit_missed" value="{{ $plan->profit_missed }}">
                        <span class="input-group-text" for="profit_missed">%</span>
                    </div>
                </div>
                <label for="desc">{{ __('locale.Description')}}</label>
                <div class="input-group mb-1">
                    <input type="text" class="form-control"  id="desc" name="desc" aria-label="Plan Description" aria-describedby="desc" value="{{ $plan->desc }}">
                </div>
                <div class="d-flex justify-content-start align-items-top mb-1">
                    <div class="me-1">
                        <img class="img-thumbnail mb-1" style="max-width: 164px" src="{{ getImage(imagePath()['forex_investment']['path'].'/'. $plan->image,imagePath()['forex_investment']['path']) }}"/>
                    </div>
                    <div>
