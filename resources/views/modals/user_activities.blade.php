    ...
    <div class="modal-dialog modal-dialog-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="popup-body popup-body-lg">
                <h3 class="popup-title">{{ __('Login Activities') }} <em class="ti ti-angle-right"></em> <small
                ...
                <ul class="data-details-alt">
                    @forelse($activities as $activity)
                        ...
                        <li class="text-dark d-md-flex justify-content-between">
                            <div>{{ __('Login with') }} <span class="text-light">{{ $browser[0] }}</span> in <span
                                    class="text-light">{{ end($device) }}</span> via <span
                                    class="text-light">{{ $ip }}</span></div>
                            <div><span class="text-light">{{ _date($activity->created_at) }}</span></div>
                        </li>
                    @empty
                        <li>
                            <div> {{ __('No Activities Found!!') }} </div>
                        </li>
                    @endforelse
                </ul>
            </div>
