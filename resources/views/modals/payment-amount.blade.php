...
<div class="popup-body">
    <h2 class="popup-title">{{ __('Buy') }} {{ strtoupper(token_symbol()) }} {{ __('Tokens') }}</h2>
    <div class="tranx-payment-details">
        <form action="{{ route('user.ajax.token.access') }}" method="POST" id="token_payment">
            ...
            <div class="payment-input">
                <input name="token_amount" placeholder="Enter number of token" class="input-bordered token-number"
                    type="number" id="token-number" value="" min="{{ active_stage()->min_purchase }}"
                    max="{{ active_stage()->max_purchase }}"><span
                    class="payment-get-cur payment-cal-cur">{{ token_symbol() }}</span>
            </div>
            <div class="gaps-1x"></div>
            <input type="hidden" class="input-checkbox" id="agree" name="agree" value="1">
            <div class="gaps-2x"></div>
            <button type="submit" class="btn  btn-alt btn-primary payment-btn">{{ __('Proceed') }}<em
                    class="ti-arrow-right"></em></button>
