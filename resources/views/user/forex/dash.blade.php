    ...
    <div class="col-xl-3 col-lg-4 col-md-12 col-sm-12">
        <div class="card card-profile">
            ...
            <div class="card-body">
                ...
                @if ($account != null)
                    @if ($account->status == 1)
                        <a href="{{ route('user.forex.trade') }}"><button type="button" class="btn btn-primary mt-1">{{ __('locale.Launch Forex App') }}</button></a>
                    @else
                        <button type="button" class="btn btn-secondary mt-1" disabled>{{ __('locale.Forex App Locked') }}</button>
                        <div class="text-warning"><small>{{ __('locale.Deposit Firstly') }}</small></div>
                    @endif
                    @if ($wallets != 'null')
                    <div class="d-flex justify-content-center align-items-center my-1">
                        <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#deposit">{{ __('locale.Deposit')}}</button>
                        <button type="button" class="btn btn-danger ms-1" data-bs-toggle="modal" data-bs-target="#withdraw">{{ __('locale.Withdraw')}}</button>
                    </div>
                    @else
                    <form class="my-1" method="POST" action="{{ route('user.wallet.create') }}">
                        @csrf
                        <input type="hidden" id="id" name="id" value="USDT">
                        <button type="submit" class="btn btn-warning">{{ __('locale.Create Wallet') }}</button></span>
                    </form>
                    @endif
                @else
                ...
                @if ($account != null)
                    ...
                    <div class="fw-bold rounded border-success p-1">{{ __('locale.Account Password') }}: <span class="text-success">{{ $account->password }}</span></div>
            ...
            <div class="modal-body flex-grow-1">
                <div class="dropdown w-auto mb-1">
                    <button class="btn btn-outline-warning dropdown-toggle" type="button"
                        data-bs-toggle="dropdown" aria-expanded="false" id="wallet1" name="wallet1">
                        ...
                        @if ($wallets != 'null')
                            ...
                            <li>
                                <a class="dropdown-item"
                                    onclick="$('#wallet1').text($(this).text());$('#deposit').find('input[name=wallet]').val($(this).data('wallet'));$('#deposit').find('span[name=amountSymbol]').text($(this).data('symbol'));"
                                    data-wallet="{{ $wallet->address }}" data-symbol="{{ $wallet->symbol }}">{{ $wallet->symbol }} {{ getAmount($wallet->balance) }}
                                </a>
                            </li>
                            @endforeach
                        @endif
    ...
    <div class="modal-dialog sidebar-sm">
        <form class="add-new-record modal-content pt-0" action="{{ route('user.forex.withdraw') }}" method="POST"
            ...
            <div class="modal-body flex-grow-1">
                ...
                <div class="dropdown w-auto mb-1">
                    <button class="btn btn-outline-warning dropdown-toggle" type="button"
                        data-bs-toggle="dropdown" aria-expanded="false" id="wallet2" name="wallet2">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        @if ($wallets != 'null')
                            ...
                            <li>
                                <a class="dropdown-item"
                                    onclick="$('#wallet2').text($(this).text());$('#withdraw').find('input[name=wallet]').val($(this).data('wallet'));"
                                    data-wallet="{{ $wallet->address }}" >{{ $wallet->symbol }} {{ getAmount($wallet->balance) }}
                                </a>
                            </li>
                            @endforeach
                        @endif
                ...
                <button type="submit" class="btn btn-primary data-submit me-1">{{ __('locale.Withdraw')}}</button>
                <button type="reset" class="btn btn-outline-secondary"
                    data-bs-dismiss="modal">{{ __('locale.Cancel')}}</button>
            </div>
        </form>
    ...
    <div class="col-xl-9 col-lg-8 col-md-12 col-sm-12">
                        ...
                        <div class="col-xl-6 col-lg-6">
                            <div class="card text-center">
                                <div class="card-body">
                                    ...
                                    <h2 class="fw-bolder">{{ $account->balance }} {{ $general->cur_text }}</h2>
                        ...
                        <div class="col-xl-6 col-lg-6">
                            <div class="card text-center">
                                <div class="card-body">
                                    ...
                                    <h2 class="fw-bolder">{{ $forex_log->sum('profit') }} {{ $general->cur_text }}</h2>
                        ...
                        <div class="col-xl-6 col-lg-6">
                            <div class="card text-center">
                                <div class="card-body">
                                    ...
                                    <h2 class="fw-bolder">{{ $investment_logs->sum('amount') }} {{ $general->cur_text }}</h2>
                        ...
                        <div class="col-xl-6 col-lg-6">
                            <div class="card text-center">
                                <div class="card-body">
                                    ...
                                    <h2 class="fw-bolder">{{ $investment_logs->where('status',1)->sum('profit') }} {{ $general->cur_text }}</h2>
                            ...
                            <div class="transaction-item">
                                <div class="d-flex">
                                    <div class="avatar bg-light-primary rounded float-start">
                                        <div class="avatar-content">
                                            @if ($forex_log->type == 1)
                                                <span class="text-success font-medium-5"><i class="bi bi-journal-arrow-up"></i></span>
                                            @elseif ($forex_log->type == 2)
                                                <span class="text-danger font-medium-5"><i class="bi bi-journal-arrow-down"></i></span>
                                            @elseif ($forex_log->type == 3)
                                                <span class="text-warning font-medium-5"><i class="bi bi-briefcase"></i></span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="transaction-percentage">
                                        @if ($forex_log->type == 1)
                                            <span class="text-success fw-bold">{{ __('locale.Forex Deposit')}}</span>
                                            </h6><br>
                                            <small> {{ $forex_log->created_at }} </small>
                                        @elseif ($forex_log->type == 2)
                                            <span class="text-danger fw-bold">{{ __('locale.Forex Withdraw')}}</span>
                                            </h6><br>
                                            <small> {{ $forex_log->created_at }} </small>
                                        @elseif ($forex_log->type == 3)
                                            <span class="text-warning fw-bold">{{ __('locale.Forex Investment')}}</span>
                                            </h6><br>
                                            <small> {{ $forex_log->created_at }} </small>
                                        @endif
                                    </div>
                                ...
                                <div class="fw-bolder">
                                    @if ($forex_log->type == 1)
                                        ...
                                        @else
                                            <span class="badge bg-success">- {{ ttz($forex_log->amount) }} {{ $general->cur_text }}</span>
                                        @endif
                                    @elseif ($forex_log->type == 2)
                                        ...
                                        @if ($forex_log->status == 0)
                                            <span class="badge bg-warning">{{ __('locale.Pending') }}</span>
                                        @else
                                            <span class="badge bg-danger">+ {{ ttz($forex_log->amount) }} {{ $general->cur_text }}</span>
                                        @endif
                                    @elseif ($forex_log->type == 3)
                                        @if ($forex_log->status != 1)
                                            <span class="badge bg-warning">{{ __('locale.Pending') }}</span>
                                        @else
                                            <span class="badge bg-warning">+ {{ ttz($forex_log->profit) }} {{ $general->cur_text }}</span>
                                        @endif
                                    @endif
                                </div>
                            </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form id="forex_investment" action="{{ route('user.forex.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                    @if ($wallets != 'null')
                                    <div class="dropdown">
                                        <button class="btn btn-outline-warning dropdown-toggle w-100 mb-1" type="button"
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <li>
                                                    <a class="dropdown-item"
                                                        onclick="$('#wallett').text($(this).text());$('#forex_investment').find('input[name=wallet]').val($(this).data('wallet'));"
                                                        data-wallet="{{ $wallet->address }}">{{ $wallet->symbol }} {{ getAmount($wallet->balance) }}
                                                    </a>
                                                </li>
                                                @endforeach
                                    ...
                                    <form method="POST" action="{{ route('user.wallet.create') }}">
                                        @csrf
                                        <input type="hidden" id="id" name="id" value="USDT">
                                        <button type="submit" class="btn btn-warning w-100">{{ __('locale.Create Wallet') }}</button></span>
                                    </form>
                                    @endif
                                </div>
                                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                ...
                                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                    <button type="button" class="btn btn-primary w-100 mb-1" data-bs-toggle="modal" data-bs-target="#InvestmentType" id="selectInvestment">{{ __('locale.Select Investment Plan')}}</button>
                                </div>
                                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                                    <button
                                        class="w-100 mb-1 btn btn-success d-flex align-items-center justify-content-start"
                                        type="submit"><i class="bi bi-caret-right me-1"></i><span>{{ __('locale.Start Investment')}}</span>
                                    </button>
                                </div>
                            </div>
                        </form>
                        ...
                        <table class="table">
                            <thead class="table-dark">
                                ...
                                <tr>
                                    ...
                                    <td data-label="{{ __('locale.Plan')}}" class="text-uppercase">{{ getForexInvestment($investment_log->investment_id)->title }}</td>
                                    <td data-label="{{ __('locale.Amount')}}">{{__(getAmount($investment_log->amount))}} $</td>
                                    <td data-label="{{ __('locale.Profit')}}" class="@if ($investment_log->result == 1) text-success
                                        @elseif($investment_log->result == 2) text-danger @elseif($investment_log->result == 3) @else @endif">
                                        @if ($investment_log->result == 1) + {{__(getAmount($investment_log->profit))}} $
                                        @elseif($investment_log->result == 2) - {{__(getAmount($investment_log->profit))}} $
                                            @elseif($investment_log->result == 3) {{__(getAmount($investment_log->profit))}} $
                                            @else <span class="badge bg-warning">{{ __('locale.Pending')}}</span> @endif</td>
                                    <td data-label="{{ __('locale.Duration')}}">
                                        <div> {{ __('locale.Start')}}: <span class="fw-bold">{{showDateTime($investment_log->start_date, 'd M, Y h:i:s')}}</span></div>
                                        <div> {{ __('locale.End')}}: <span class="fw-bold">{{showDateTime($investment_log->end_date, 'd M, Y h:i:s')}}</span></div>
                                    </td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body pb-3 px-sm-3">
                ...
                <a onclick="$('#forex_investment').find('input[name=investment_id]').val($(this).data('id'));$('#selectInvestment').text($(this).data('investment'));$('#InvestmentType').modal('hide');"
                    style="stretched-link" data-id="{{ $investment->id }}" data-investment="{{ $investment->title }}">
                    <div class="row bg-wallet p-1 rounded mb-1 @if($investment->id == 1) bg-wallet-active @endif">
                        <div class="col-2">
                                <img class="rounded-circle shadow-4"
                                style="width: 64px;" src="{{ getImage('assets/images/forex/investment/'.$investment->image)}}">
                        </div>
                        <div class="col-10">
                            <div class="d-flex justify-content-between">
                                <div class="fw-bold fs-4 text-white">{{ $investment->title }}
                                    @if ($investment->is_new == 1)<span class="fs-6 badge bg-success text-white">{{ __('locale.New')}}</span>@endif
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div>{{ __('locale.Return on Investment')}}: <span class="text-success">{{ ttz($investment->roi) }}%</span></div>
                            </div>
                        </div>
                    </div>
                </a>
...
@if ($account != null)
<div class="row">
@if($account->signal1_id != NULL||$account->signal2_id != NULL||$account->signal3_id != NULL||$account->signal4_id
    != NULL||$account->signal5_id != NULL)
