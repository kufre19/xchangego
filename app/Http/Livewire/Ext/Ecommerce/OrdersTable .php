...
{
    protected $model = EcommerceOrders::class;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Actions", "id")
                ->view('extensions.admin.ecommerce.order.actions'),
        ];
    }
    ...
    public function activate()
    {
        ...
        foreach ($this->getSelected() as $id) {
            $item = EcommerceOrders::where('id', $id)->first();
            $names .= $item->name . ', ';
            $item->status = 1;
            $item->save();
        }
        Session::flash('alert', [
            ...
            'header' => 'Alert!',
            'message' =>  '(' . rtrim($names, ", ") . ') ' . ($count > 1 ? 'Orders' : 'order') . ' Activated Successfully'
        ]);
    ...
    public function deactivate()
    {
        ...
        Session::flash('alert', [
            ...
            'header' => 'Alert!',
            'message' =>  '(' . rtrim($names, ", ") . ') ' . ($count > 1 ? 'Orders' : 'Order') . ' Deactivated Successfully'
        ]);
        $this->clearSelected();
    }
    ...
    public function delete()
    {
        $names = '';
        $count = count($this->getSelected());
        $path = imagePath()['ecommerce_order']['path'];
        foreach ($this->getSelected() as $id) {
            $item = EcommerceOrders::where('id', $id)->first();
            $names .= $item->name . ', ';
            if ($item->icon != null) {
        ...
        Session::flash('alert', [
            ...
            'icon' => 'exclamation-triangle',
            'header' => 'Alert!',
            'message' =>  '(' . rtrim($names, ", ") . ') ' . ($count > 1 ? 'Orders' : 'Order') . ' Removed Successfully'
        ]);
        $this->clearSelected();
    }
}
